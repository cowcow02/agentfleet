...
import { createDispatch, completeDispatch, appendDispatchMessage, serializeDispatch } from "../dispatch";
...
describe("dispatch", () => {
  ...
  describe("createDispatch", () => {
    it("finds agent, inserts row, sends WS message, emits event", async () => {
      ...
      const result = await createDispatch("org1", {
        ticketRef: "TICK-1",
        title: "Fix bug",
        description: "desc",
        labels: ["ts"],
  ...
  describe("completeDispatch", () => {
    it("updates status and converts duration_seconds to duration_ms", async () => {
      const updatedRow = makeFakeDispatchRow({ status: "completed", exitCode: 0, durationMs: 5500 });
  ...
  describe("appendDispatchMessage", () => {
    it("appends message to existing dispatch", async () => {
      ...
      mockSelect
        ...
        .mockResolvedValueOnce([{ ...existingRow, messages: [{ message: "old", timestamp: "t0" }, { message: "new", timestamp: "t1" }] }]);
